# Build cross-area test set in UAV-Visloc based on AMS
        ...
        with tqdm(total=total_tiles, desc="Building the satellite database") as pbar:
            for top in top_list:
                for left in left_list:
                    ...
                    lat = LT_lat - center_y * lat_per_pixel
                    lon = LT_lon + center_x * lon_per_pixel
...
if __name__ == "__main__":
    ...
    target_size = (3,3) # Scaling satellite image resolution

    satellite_id = [1,2]

    # Ground sample distance (GSD) may have errors
    space_resolution = [0.1,0.1,0.11]
    ...
    for i in range(1,10):
        ...
        scales_path = os.path.join(out_path, f'scales.csv')
        ...
        for id in range(len(satellite_id)):
            ...
            image_path = os.path.join(data_root, f'{satellite_id[id]:02}/satellite{satellite_id[id]:02}.tif')
