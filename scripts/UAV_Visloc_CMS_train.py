# Build UAV-Visloc training set based on CMS
...
def crop_tif_to_tiles(tif_path, out_dir, id, coord_range, uav_coords, tile_size=512, record_txt_path=None, target_size=None):
        ...
        with tqdm(total=len(uav_coords), desc="Building the 1v1 satellite database") as pbar:
            for lat, lon in uav_coords:
                ...
                if top < 0 or left < 0 or top + tile_size > height or left + tile_size > width:
                    print(f"Skipping tile at ({lat},{lon}) - out of bounds")
...
if __name__ == "__main__":
    ...
    for i in range(1,10):
        ...
        if Crop:
            ...
            crop_tif_to_tiles(image_path, out_path, id=i, coord_range=coord_range, uav_coords=list(drone_coordinates), tile_size=tile_size, record_txt_path=satellite_coordinates_path, target_size=target_size)
