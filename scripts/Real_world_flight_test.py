...
def calcultae_scale(d,T):
    # lamda: float = 2 * d / T
    # scale = 1 + 0.2 * math.tanh(lamda **2) **2
    scale = d / T
    return round(scale, 4)

def compute_tile_count(W, H, T, S):
    n_w = math.ceil((W - T) / S) + 1
    n_h = math.ceil((H - T) / S) + 1
    return n_w * n_h
...
def crop_tif_to_tiles(tif_path, out_dir, id, tile_size=512, stride=256, tile_id = 1, record_txt_path=None, target_size=None):
        ...
        with tqdm(total=total_tiles, desc="Building the satellite database") as pbar:
            for top in top_list:
                for left in left_list:
                    ...
                    tile = src.read(window=window)  # shape: (bands, tile_size, tile_size)
                    ...
                    old_tile_name = f"{id:02}_{tile_id:04d}.tif"
                    new_tile_name = f"{id:02}_{tile_id:05d}.tif"

                    old_out_path = os.path.join(out_dir, old_tile_name)
...
if __name__ == "__main__":
    ...
    tile_size_set = None # None

    stride_set = None # None

    Crop = True

    target_size = (392,392) # None | (392,392)
    ...
    for i in range(1,4):
        ...
        for id in range(len(satellite_id)):
            ...
            if Crop:
                ...
                tile_id = crop_tif_to_tiles(image_path, out_path, id=i, tile_size=tile_size, stride=stride_mean, tile_id = current_tile_id, record_txt_path=satellite_coordinates_path, target_size=target_size)
